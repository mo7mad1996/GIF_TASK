<template>
  <nuxt-link :to="`/gif/${post.id}?from=tenor`" class="link_tenor">
...
<script>
// components
import Load from "~/components/home/TenorCard/load";
      ...
      if (this.liked) {
        ...
        if (this.getCookie("favourite")) {
          let lastData = JSON.parse(this.getCookie("favourite")),
            newData = lastData.filter((el) => ele.id != this.post.id);
...
.link_tenor .favourite {
  position: absolute;
  bottom: -70px;
  right: 10px;
  transition: 0.4s;
}
.link_tenor .favourite button {
